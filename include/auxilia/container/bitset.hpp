...
namespace std {
template <size_t N> struct hash<::auxilia::bitset<N>> {
  AC_NODISCARD AC_STATIC_CALL_OPERATOR constexpr auto
  operator()(const ::auxilia::bitset<N> &bs) AC_CONST_CALL_OPERATOR noexcept {
    size_t seed = 0;
    using storage_type = typename ::auxilia::bitset<N>::storage_type;
    constexpr auto arr_len = ::auxilia::bitset<N>::arr_length;
    hash<storage_type> hasher;
    constexpr auto my_magic_number = sizeof(storage_type) == 4
                                         ? ::auxilia::hash_magic_number_32bit
                                         : ::auxilia::hash_magic_number_64bit;
