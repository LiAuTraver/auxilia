...
TEST(Options, Parsing) {
  ...
  parser.add_option("--my-help", "-m", "Show help message").nargs(0);
...
TEST(Options, Required) {
  auto parser = Local("test", "1.0");
  parser.add_option("--input", "-i", "Input file").nargs('1').required();
...
TEST(Options, Positional) {
  auto parser = Local("test", "1.0");
  parser.add_option("--other", "-t").nargs(0);
...
TEST(Options, Default) {
  ...
  p1.add_option("--level", "-l").nargs('1').default_value("info");
...
TEST(Options, HelpAndVersion) {
  ...
  parser.add_option("--input", "-i", "Input file").nargs('1');
...
TEST(Options, Reserved) {
  auto parser = Local("test", "1.0");
  parser.add_option("--my-help", "-h");
  EXPECT_EQ(parser.error(), 1);
  EXPECT_NE(parser.error_messages()[0].find("'-h' is reserved"),
            std::string::npos);

  auto p2 = Local("test2", "1.0");
  p2.add_option("--version", "-v");
...
TEST(Options, Conflicting) {
  auto parser = Local("test", "1.0");
  parser.add_option("--option", "-o");
  parser.add_option("--option", "-x"); // Conflicting fullname
  EXPECT_EQ(parser.error(), 1);
  EXPECT_NE(parser.error_messages()[0].find("conflicts with existing option"),
            std::string::npos);

  auto p2 = Local("test2", "1.0");
  p2.add_option("--option1", "-o");
  p2.add_option("--option2", "-o"); // Conflicting shortname
  EXPECT_EQ(p2.error(), 1);
