...
void tcp_client() {
  ...
  s.connect(tcp_ep).log();
  s.send_bytes("HIIII!").log(logger);
  logger->info("sent!");
  std::this_thread::sleep_for(1s);
...
void udp_client() {
  ...
  s.send_bytes("HIIII", udp_ep).log(logger);
  logger->info("sent!");
  std::this_thread::sleep_for(1s);

  s.send_bytes("another msg").log(logger);
